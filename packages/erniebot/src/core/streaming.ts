...
export class Stream<Item> implements AsyncIterable<Item> {
  ...
  static fromSSEResponse<Item>(response: Response, controller: AbortController) {
    ...
    async function* iterMessages(): AsyncGenerator<ServerSentEvent, void, unknown> {
      if (!response.body) {
        controller.abort()
        throw new EBError(`Attempted to iterate over a response with no body`)
      }
  ...
  toReadableStream(): ReadableStream {
    ...
    return new ReadableStream({
      ...
      async pull(ctrl: ReadableStreamController<any>) {
        try {
...
class LineDecoder {
  ...
  decodeText(bytes: Bytes): string {
    ...
    throw new EBError(`Unexpected: neither Buffer nor TextDecoder are available as globals. Please report this error.`)
  }
